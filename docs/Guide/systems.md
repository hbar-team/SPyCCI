...
* [`ReactionPath`: a collection of closely related `System` objects.](reactionpath-section) For example, a set of intermediate and transition states structures (e.g. from reactants to products).
* [`Ensemble`: a collection of closely related `System` objects representing a physical ensemble.](ensamble-section) For example, using canonical NVT or microcanonical NVE formalism.
