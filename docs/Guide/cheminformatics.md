...
assigned to same atom?"]
	...
	RadicalsAffected [label="Are set radicals affected?"]
	Start -> HasMetals
...
on ligand only" color=darkgreen dir=both fontcolor=darkgreen style=dashed]
	...
	RadicalsAssigned -> SanitizeProps [label=NO]
	SanitizeProps -> End
	RadicalsAssigned -> TrySanitizeProps [label=YES]
	...
	AdjustRadicalConnectivity -> SanitizeFinal
	SanitizeFinal -> End
}
```
