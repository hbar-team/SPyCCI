...
def packmol_cube(
    ...
    if nsolv and target_dens and cube_side:
        ...
        solvent_grams = (
            sum([atomic_masses[atom] for atom in solvent.geometry.get_atoms()]) * nsolv / avogadro
        )  # g
        solute_grams = (
            sum([atomic_masses[atom] for atom in solute.geometry.get_atoms()]) / avogadro
        )  # g
        ...
        nsolv = round(
            target_solv_weight
            * avogadro
            / sum([atomic_masses[atom] for atom in solvent.geometry.get_atoms()])
        )  # n° of molecules
