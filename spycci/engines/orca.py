...
class OrcaJobInfo:
    ...
    @property
    def irc_block(self) -> Dict[str, Any]:
        """
        Returns the %irc block generated by merging the user provided irc options with the ones specified by the library
        flags.

        Returns
        -------
        Dict[str, Any]
            The content of the irc block encoded in dictionary format.
        """

        block = {} if "irc" not in self.user_blocks else self.user_blocks["irc"]

        if self.irc:
            block["MaxIter"] = f'{self.irc_maxiter}'
            block["Direction"] = f'{self.irc_direction}'
            block["hessMode"] = f'{self.irc_hess_mode}'
            block["InitHess"] = f'{self.irc_init_hess}'

            if self.irc_hess_filename:
                block["Hess_Filename"] = f'{self.irc_hess_filename}'
    ...
    def parsed_blocks(self) -> Dict[str, Dict[str, Any]]:
        ...
        if self.irc_block != {}:
            blocks["irc"] = self.irc_block

        for key, value in self.user_blocks.items():

            if key not in ["geom", "cpcm", "plots", "output", "elprop", "neb", "irc"]:
                blocks[key] = value
...
class OrcaInput(Engine):
    ...
    def irc(
        ...
        direction: str = 'both',
        hess_mode: int = 0,
        init_hess: str = 'calc_anfreq',
        hess_filename: str = None,
        ncores: int = None,
        maxcore: int = 750,
        remove_tdir: bool = True,
        blocks: Optional[Dict[str, Dict[str, Any]]] = None,
        ...
        """
        Run an Intrinsic Reaction Coordinate (IRC) calculation starting from an optimized
        transition state structure, and return the reaction path corresponding to the
        minimum energy path connecting the reactant and product geometries.

        The IRC is computed by following the potential energy surface downhill from the
        transition state in one or both directions along the reaction coordinate.

        Arguments
        ---------
        transition_state : System
            The optimized transition state structure to be used as the starting point
            for the IRC calculation.
        maxiter : int, optional
            Maximum number of optimization steps to perform in each direction of the IRC (default: 20).
        direction : str, optional
            Direction of the IRC calculation. Possible values are:
                - ``'both'`` – run the IRC in both forward and backward directions (default).
                - ``'forward'`` – follow the path toward the product/reactant only.
                - ``'backward'`` – follow the path toward the reactant/product only.
                - ``'down'`` – follow the reaction coordinate downhill in the gradient direction.
        hess_mode : int, optional
            Number of the eigenvector of the eigenmode with the lowest frequency to be followed 
            for the initial displacement. Default is 0 (the mode with the lowest frequency, 
            typically the imaginary mode at the transition state).
        init_hess : str, optional
            Method used for initializing the Hessian. Possible values are:
                - ``'calc_anfreq'`` – compute the analytic Hessian (default).
                - ``'calc_numfreq'`` – compute the numerical Hessian.
                - ``'read'`` – read the Hessian from a file (must be used with ``hess_filename``).
        hess_filename : str, optional
            Path to the input Hessian file to be used when ``init_hess='read'``.
        ncores : int, optional
            number of cores, by default all available cores
        maxcore : int, optional
            memory per core, in MB, by default 750
        ...
        blocks : Optional[Dict[str, Dict[str, Any]]]
            The dictionary of dictionaries encoding a series of custom blocks defined by the user. If set to a non-empty
            value, will overwrite the block option eventually set on the `OrcaInput` class construction.

        Raises
        ------
        RuntimeError
            Raised if the given `System` object is not suitable for an IRC calculation
            (e.g. invalid or missing transition state data).

        Returns
        -------
        ReactionPath
            The reaction path object containing the sequence of optimized structures
            along the computed intrinsic reaction coordinate.

        Examples
        --------
        :BASIC IRC CALCULATION:

        Starting from an initialized instance ``orca`` of the ``OrcaInput`` class and
        a valid transition state ``ts`` structure, an IRC calculation can be run with:

        >>> reaction_path = orca.irc(ts)

        The returned ``reaction_path`` object is of type ``ReactionPath`` and contains
        the sequence of geometries along the intrinsic reaction coordinate.

        :CUSTOM IRC SETTINGS:

        To perform an IRC only in the forward direction with a precomputed Hessian:

        >>> reaction_path = orca.irc(
        ...     ts,
        ...     direction='forward',
        ...     init_hess='read',
        ...     hess_filename='ts_freq.hess',
        ... )

        **Note:**  
        The transition state structure must be a valid stationary point with exactly
        ...
        with sh.pushd(tdir):
            ...
            transition_state.geometry.write_xyz(f"{transition_state.name}.xyz")

            job_info = OrcaJobInfo()
            job_info.ncores = ncores
            job_info.maxcore = maxcore
            job_info.is_singlet = True if transition_state.spin == 1 else False
            job_info.solvent = self.solvent
            job_info.irc = True
            job_info.irc_maxiter = maxiter
            job_info.irc_direction = direction
            job_info.irc_hess_mode = hess_mode
            job_info.irc_init_hess = init_hess
            job_info.irc_hess_filename = hess_filename

            job_info.user_blocks = blocks if blocks else self.blocks

            self.write_input(mol=transition_state, job_info=job_info)
    ...
    def cosmors(
        ...
        if use_engine_settings:
            ...
            logger.warning(
                "OpenCOSMO-RS has been parametrized using BP86/def2-TZVPD, running calculations with other levels of theory is not advisable."
            )
        else:
        ...
        if not locate_orca(f"{self.__ORCAPATH}", ">=6.0.0"):
