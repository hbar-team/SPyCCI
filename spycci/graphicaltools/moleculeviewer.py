...
# Default CUBE colors
CUBE_POSITIVE_COLOR = "#FF6600"
CUBE_NEGATIVE_COLOR = "#0238CF"
...
def show_molecule(
    ...
    cube: Optional[Cube]
        The `Cube` object encoding the volumetric data to visualized. If `None` (default)
        the volumetric data rendering will be skipped.
    ...
    if cube is not None:
        ...
        nx, ny, nz = cube.nvoxels
        x_axis, y_axis, z_axis = cube.axes
        origin = cube.origin

        # Compute coordinate vectors
        x = origin[0] + np.arange(nx) * x_axis[0]
        y = origin[1] + np.arange(ny) * y_axis[1]
        z = origin[2] + np.arange(nz) * z_axis[2]

        # Create structured grid
        grid = pv.StructuredGrid(*np.meshgrid(x, y, z, indexing="ij"))
        grid["values"] = cube.cube.flatten(order="F")
        ...
        # Extract isosurface
        positive_surface = grid.contour([isovalue])
        negative_surface = grid.contour([-isovalue])

        # Plot positive and negative isosurfaces if they are not empty
        if positive_surface.number_of_points > 0:
            plotter.add_mesh(positive_surface, color=CUBE_POSITIVE_COLOR, opacity=0.8, name="positive isosurface")
        
        if negative_surface.number_of_points > 0:
            plotter.add_mesh(negative_surface, color=CUBE_NEGATIVE_COLOR, opacity=0.8, name="negative isosurface")
